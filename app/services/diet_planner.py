...
class DietPlannerService:
    ...
    def _generate_llm_prompt(self, target_calories):
        # Get temporary data from the incoming request (Postman)
        ...
        cuisine_prompt = "The diet should be primarily Indian, using commonly available ingredients in India."
        if optional_cuisines:
            cuisine_prompt += f" However, please incorporate 2-3 meals from {optional_cuisines} throughout the week for variety."

        prompt = f"""
        Please generate a comprehensive, 7-day weekly diet plan.
        ...
        - Disliked Foods: {self.user.disliked_foods or 'None'}
        - Allergies: {self.user.allergies or 'None'}
